...
public abstract class ColorButton extends ComponentButton
{
	...
	public final GuiMesh button = new GuiMesh();
	public final GuiMesh color = new GuiMesh();
	...
	public void setSize(int x1, int y1, int x2, int y2)
	{
		...
		if (button != null) button.setMesh(x1, y1, x2, y2, 0, 0, 1, 1);
		if (color != null) color.setMesh(x1 + 1, y1 + 1, x2 - 1, y2 - 1, 0, 0, 1, 1);
	...
	public void render(ButtonState state)
	{
		...
		Main.instance.guiShader.setColor(new Color(getColor(), 1));
		color.render();
		Main.instance.guiShader.setColor(1f, 1f, 1f, 1f);
	}
