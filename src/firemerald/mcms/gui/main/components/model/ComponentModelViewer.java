...
public class ComponentModelViewer extends Component
{
	...
	public void tick(float mx, float my, float deltaTime)
	{
    	...
    	if (this.contains(mx, my))
    	{
    		...
    		if (model != null)
    		{
            	...
            	p2.mul(1 / p2.w());
            	main.trace = model.rayTrace(p1.x(), p1.y(), p1.z(), p2.x() - p1.x(), p2.y() - p1.y(), p2.z() - p1.z(), model.getPose(project.getAnimation() == null ? new AnimationState[0] : new AnimationState[] {new AnimationState(project.getAnimation(), main.animTime)}));
	...
	public void render(float mx, float my, boolean canHover)
	{
    	...
    	if (model != null)
    	{
        	Map<String, Matrix4d> map = model.getPose(project.getAnimation() == null ? new AnimationState[0] : new AnimationState[] {new AnimationState(project.getAnimation(), main.animTime)});
        	project.bindTex();
    	...
    	else if (state.showNodes() || state.showBones()) //skeleton view
    	{
    		...
    		if (skeleton != null)
    		{
            	Map<String, Matrix4d> map = skeleton.getPose(project.getAnimation() == null ? new AnimationState[0] : new AnimationState[] {new AnimationState(project.getAnimation(), main.animTime)});
		...
		else
		{
			...
			if (model != null)
			{
				if (main.getEditorMode() == EditorMode.TEXTURE || button == MouseButtons.LEFT)
				{
			    	...
			    	Vector4f p2 = MathUtils.toVector4f(inverse.transform(new Vector4d(rmx, rmy, 1, 1)));
			    	p2.mul(1 / p2.w());
			    	RaytraceResult trace = model.rayTrace(p1.x(), p1.y(), p1.z(), p2.x() - p1.x(), p2.y() - p1.y(), p2.z() - p1.z(), model.getPose(project.getAnimation() == null ? new AnimationState[0] : new AnimationState[] {new AnimationState(project.getAnimation(), main.animTime)}));
		...
		else
		{
			...
			if (model != null)
			{
				if (Main.instance.getEditorMode() == EditorMode.TEXTURE)
				{
			    	...
			    	Vector4f p2 = MathUtils.toVector4f(inverse.transform(new Vector4d(rmx, rmy, 1, 1)));
			    	p2.mul(1 / p2.w());
			    	RaytraceResult trace = model.rayTrace(p1.x(), p1.y(), p1.z(), p2.x() - p1.x(), p2.y() - p1.y(), p2.z() - p1.z(), model.getPose(project.getAnimation() == null ? new AnimationState[0] : new AnimationState[] {new AnimationState(project.getAnimation(), Main.instance.animTime)}));
			    	if (trace != null)
			    	{
			    		if (trace instanceof TextureRaytraceResult)
			    		{
			    			TextureRaytraceResult texRes = (TextureRaytraceResult) trace;
			    			if (texRes.tex != null) Main.instance.tool.onMouseDrag(texRes.tex, prevU, prevV, prevU = texRes.u, prevV = texRes.v, button);
			    		}
			    	}
				}
	...
	@Override
	public void onKeyPressed(Key key, int scancode, int mods) //TODO invert?
	{
		if (mods == 0)
		{
