...
public class HotKey
{
	...
	public HotKey(AbstractElement el)
	{
		...
		if (el.hasAttribute("modifiers")) for (String mod : el.getString("modifiers", "").split("\\*"))
		{
			...
			if (!flag)
			{
				Main.LOGGER.warn("Couldn't load HotKey with invalid modifier " + mod);
				key = Key.UNKNOWN;
				modifiers = 0;
				break;
			}
	...
	public void writeToElement(AbstractElement el)
	{
		...
		{
			Main.LOGGER.warn("Couldn't load HotKey with invalid modifier flags " + Integer.toBinaryString(this.modifiers - modsCheck));
			return;
		}
		el.setEnum("key", key);
